    ...
    for(i32 z = 0; z < world->chunks_size.z; z++) {
        for(i32 x = 0; x < world->chunks_size.x; x++) {
            for(i32 y = 0; y < world->chunks_size.y; y++) {
                ...
                if(hashmap_get(&world->columns_in_generation, &(Vec2i) {
                {
...
Chunk **world_generate_chunk_column(Vec2i *column_position)
{
    ...
    for(i32 i = 0; i < CHUNK_COLUMN_HEIGHT; i++) {
        ...
        chunk_create(column[i], &(Vec3i) {
            {
                column_position->x, i, column_position->y
            }
        });
    }
    ...
    for(i32 z = 0; z < CHUNK_SIZE; z++) {
        for(i32 x = 0; x < CHUNK_SIZE; x++) {
            Vec2i block_pos;
            zinc_vec2i_add(&column_pos_in_blocks, &(Vec2i) {
                {
                    x, z
                }
            }, &block_pos);

            f32 mountain_noise = noise_2d_octave_perlin(&world->noise, &(Vec2) {
                {
                    block_pos.x / 900.0f, block_pos.y / 900.0f
                }
            }, 3, 0.5f);
            ...
            for(i32 y = CHUNK_COLUMN_HEIGHT * CHUNK_SIZE - 1; y >= 0; y--) {
                ...
                f32 noise_3d = noise_3d_octave_perlin(&world->noise, &(Vec3) {
                    {
                        block_pos.x / 100.0f, y / 800.0f, block_pos.y / 100.0f
                    }
                }, 3, 0.3);
                    ...
                    Chunk **chunk, Vec3i *block_offset, Direction *facing_dir)
{
    ...
    while(curr_dist <= max_distance) {
        ...
        if(*chunk) {
            u16 block_id = (*chunk)->block_data->data[CHUNK_OFFSET_2_INDEX(
                               *block_offset)] & BLOCK_ID_MASK;
