...
void cyclic_queue_resize(CyclicQueue *queue, u64 new_size)
{
    ...
    if(queue->allocated_elements < new_size) {
        ...
        if(added_elements_size >= queue->end) {
            memcpy(queue->data + old_size * queue->element_size, queue->data,
                   queue->end * queue->element_size);
            queue->end = (queue->start + queue->size) % new_size;
        }
        else {
            memcpy(queue->data + old_size * queue->element_size, queue->data,
                   added_elements_size * queue->element_size);
            queue->end -= added_elements_size;
            memmove(queue->data, queue->data + added_elements_size * queue->element_size,
                    queue->end);
        }
    ...
    if(queue->end > queue->start) {
        memmove(queue->data, queue->data + queue->start * queue->element_size,
        ...
        memcpy(new_data, queue->data + queue->start * queue->element_size,
               elements_left_of_start * queue->element_size);
        memcpy(new_data + elements_left_of_start * queue->element_size, queue->data,
               queue->end);
        free(queue->data);
        queue->data = new_data;
    }
