...
inline void array_list_destroy(ArrayList *list)
{
    free(list->data);
}
...
void array_list_remove(ArrayList *list, u64 index, void (*free_element)(void *))
{
    ...
    element = array_list_offset(list, index);
    if (free_element)
        free_element(element);

    next_element = array_list_offset(list, index + 1);
...
{
    void *element_to_remove, *last_element;
    ...
    element_to_remove = array_list_offset(list, index);
    if (free_element)
        free_element(element_to_remove);

    last_element = array_list_offset(list, list->size - 1);
